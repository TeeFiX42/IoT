...
class RadiationWatch:
    ...
    def __init__(self, radiation_pin, noise_pin, shutdown_pin, display_pin, wifi_pin, numbering=GPIO.BCM):
    ...
    def register_wifi_callback(self, callback):
        """Register a function that will be called on wifi button occurrence. """
        self.wifi_callback = callback

    def register_display_callback(self, callback):
        """Register a function that will be called on display button occurrence. """
        self.display_callback = callback

    def register_shutdown_callback(self, callback):
        """Register a function that will be called on shutdown occurrence. """
        self.shutdown_callback = callback

    def register_reset_callback(self, callback):
        """Register a function that will be called on reset occurrence. """
    ...
    def setup(self):
        ...
        GPIO.setup(self.wifi_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(self.display_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(self.shutdown_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)

        # Register local callbacks.
        GPIO.add_event_detect(self.radiation_pin, GPIO.FALLING, callback=self._on_radiation)
        GPIO.add_event_detect(self.noise_pin, GPIO.FALLING, callback=self._on_noise)
        GPIO.add_event_detect(self.wifi_pin, GPIO.FALLING, callback=self._on_wifi)
        GPIO.add_event_detect(self.display_pin, GPIO.FALLING, callback=self._on_display)
        GPIO.add_event_detect(self.shutdown_pin, GPIO.BOTH, callback=self._on_shutdown)
    ...
    def _on_wifi(self, _channel):
        if self.wifi_callback:
            self.wifi_callback()

    def _on_display(self, _channel):
        if self.display_callback:
            self.display_callback()

    def _on_shutdown(self, _channel):
        if GPIO.input(self.shutdown_pin):
            # rising edge, so shutdown function
            if self.shutdown_callback:
                self.shutdown_callback()
        else:
            # falling edge, so reset function
            # first reset the counters
            self.duration = 0
            self.count = 0
            # Perform the callback to the application
